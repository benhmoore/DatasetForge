...
def import_database(
    ...
    force: bool = typer.Option(
        ...
        "--force",
        "-f",
        help="Force import without confirmation, overwriting existing data.",
    ),
):
    """
        ...
        for key, target_file in target_files.items():
            if os.path.exists(target_file):
                try:
                    ...
                    typer.echo(
                        f"Warning: Could not remove existing file {target_file}: {remove_e}"
                    )
        ...
        for key, backup_file in temp_backup_files.items():
            if os.path.exists(backup_file):
                try:
                    ...
                    typer.echo(
                        f"Warning: Could not remove temp backup {backup_file}: {clean_e}"
                    )
        ...
        for key, backup_file in temp_backup_files.items():
            if os.path.exists(backup_file):
                ...
                try:
                    ...
                    typer.echo(
                        f"  Your backup might still be available at: {backup_file}"
                    )
